...
if (require.main === module) {
  run(async () => {
    ...
    switch (cmd) {
      case "init":
        if (checkVersion(cliOptions.cliVersion)) {
          await versions[cliOptions.cliVersion].init(
            cliOptions as unknown as CliFnArgs
          );
        } else {
      ...
      case "codegen":
        if (checkVersion(cliOptions.cliVersion)) {
          await versions[cliOptions.cliVersion].codegen(
            cliOptions as unknown as WithDeployKey
          );
        } else {
          throw new Error(`Unsupported version: ${cliOptions.cliVersion}`);
      ...
      case "selfupdate":
        ...
        try {
          child_process.execSync(
            "npx --yes clear-npx-cache; npx --yes @satsuma/cli ignore",
            {
              shell: "/bin/bash",
              stdio: "pipe",
            }
          ...
          console.error("Error updating Satsuma CLI.", e);
        }
        console.log("Done!");
        break;
    }
