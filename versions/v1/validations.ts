...
export const checkProjectNotExists = () => {
    ...
    if (fs.existsSync(path.resolve(process.cwd(), '.satsuma.json'))) {
        console.error('Project already exists in this directory. You must remove `.satsuma.json` before initializing');
        process.exit(1);
    }
}
...
export const validateFiles = () => {
    ...
    if (!typeDefsExist && !resolversExist) {
        console.error('No graphql types or resolvers found. Please run `init` to get started');
        process.exit(1);
    }
